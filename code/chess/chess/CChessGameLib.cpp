...
void TFigure_mass::add_figure(TFigure* figure) {
    ...
    delete[] mass;//добавить отдельный цикл дл€ удалени€
    mass = tmp_mass;
    count++;
}
TFigure_mass::TFigure_mass() {
...
//конструктор копирровани€ и деструктор дописать
...
TGame::TGame(TGame& tmp) {
    //mass = tmp.mass; нет перегрузки = дл€ TFigure_mass, добавть.
...
bool TGame::check_castling(TFigure* king) {
    //if(проверка на то, что король и ладь€ не ходили){return false;}
